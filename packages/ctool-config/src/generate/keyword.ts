...
export const buildKeywords = () => {
        ...
        tool.features.forEach(feature => {
            ...
            localesReal.forEach(code => {
                ...
                if (code === 'zh_CN') {
                    items.forEach(item => {
                        if (/[\u4e00-\u9fa5]/.test(item)) {
                            keywordItem.search.push(pinyin(item, {toneType: 'none', type: 'array', v: true}).join("")) // 全拼
                            keywordItem.search.push(pinyin(item, {pattern: "first", toneType: 'none', type: 'array', v: true}).join("")) // 首字母
                        }
                    })
                }
            })
    ...
    buildData.append(`export const toolKeywords:{ name: ToolType, feature: FeatureType, keyword: string[], search: string[] }[] = ${JSON.stringify(keywords, null, 4)}`, '工具关键字')
}
