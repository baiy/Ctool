    ...
    <Display :position="useInput ? 'right-center' : 'top-right'" :style="style">
        ...
        <Card v-else style="height: 100%">
            <Align direction="vertical" horizontal="center" vertical="center">
                ...
                <div style="font-size: .75rem;color: var(--ctool-placeholder-text-color);display: inline-flex;align-items:center;gap: 5px">
        ...
        <template #extra>
            <Align>
                ...
                <Select
                    ...
                    @change="(value)=>{current.type = value;current.value = ``;current.text = Text.empty()}"
...
watch(() => current.text, () => emit('update:modelValue', toRaw(current) as TextInput), {immediate: true, deep: true})
