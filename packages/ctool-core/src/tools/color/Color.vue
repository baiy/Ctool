<template>
    <div v-row="`auto-1`">
        ...
        <Align direction="vertical">
            <Display position="right-center" v-for="i of typeLists" :text="i" @click="$copy(colorInstance[`${i}`].value)">
                <Input v-model="colorInstance[`${i}`].value" :placeholder="$t('color_input_placeholder',[i,example[i]])" :size="size"/>
            </Display>
        </Align>
    </div>
</template>
...
const handler = (type: string) => {
    return {
        get: () => getHandle(type),
        set: (value) => setHandle(type, value)
    }
}
