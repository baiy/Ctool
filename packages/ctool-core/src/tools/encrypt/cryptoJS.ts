...
// aes
export const aes = {
    encrypt: (input: string, {key, iv = "", mode = "CBC", key_size = "128", padding = "Pkcs7", type = "simple", fill = true}: Option) => {
        if (type === 'simple') {
            return CryptoJS.AES.encrypt(CryptoJS.enc.Base64.parse(input), key).toString()
        }
        return CryptoJS.AES.encrypt(CryptoJS.enc.Base64.parse(input), secretHandle('key', key, keySizeMap[key_size] / 8), {
            mode: modeMap[mode],
            padding: paddingMap[padding],
            ...(mode === "ECB" ? {} : {iv: secretHandle('iv', iv, 16, fill)})
        }).toString()
    },
    decrypt: (input: string, {key, iv = "", mode = "CBC", key_size = "128", padding = "Pkcs7", type = "simple", fill = true}: Option) => {
        if (type === 'simple') {
...
// des
export const des = {
    encrypt: (input: string, {key, iv = "", mode = "CBC", padding = "Pkcs7", type = "simple", fill = true}: Omit<Option, "key_size">) => {
        if (type === 'simple') {
    ...
    decrypt: (input: string, {key, iv = "", mode = "CBC", padding = "Pkcs7", type = "simple", fill = true}: Omit<Option, "key_size">) => {
        if (type === 'simple') {
...
export const tripleDES = {
    encrypt: (input: string, {key, iv = "", mode = "CBC", padding = "Pkcs7", type = "simple", fill = true}: Omit<Option, "key_size">) => {
        ...
        return CryptoJS.TripleDES.encrypt(CryptoJS.enc.Base64.parse(input), secretHandle('key', key, 8), {
            mode: modeMap[mode],
            padding: paddingMap[padding],
            ...(mode === "ECB" ? {} : {iv: secretHandle('iv', iv, 8, fill)})
        }).toString()
    ...
    decrypt: (input: string, {key, iv = "", mode = "CBC", padding = "Pkcs7", type = "simple", fill = true}: Omit<Option, "key_size">) => {
        ...
        return CryptoJS.TripleDES.decrypt(input, secretHandle('key', key, 8), {
            mode: modeMap[mode],
            padding: paddingMap[padding],
            ...(mode === "ECB" ? {} : {iv: secretHandle('iv', iv, 8, fill)})
        }).toString(CryptoJS.enc.Base64)
