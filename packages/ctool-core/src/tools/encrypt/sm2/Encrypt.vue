        ...
        <Align direction="vertical">
            ...
            <Card :title="$t(`main_ui_config`)" class="ctool-page-option">
                <Align horizontal="center">
                    <Input v-model="action.current.option.public_key" placeholder="Public Key"/>
                    <Input v-model="action.current.option.private_key" placeholder="Private Key"/>
                </Align>
                <template #extra>
                    <Align>
                        <Select
...
const output = $computed<Text>(() => {
    ...
    try {
        ...
        return Text.fromHex(sm2.doEncrypt(action.current.input.text.toString(), action.current.option.public_key, action.current.option.cipher_mode as CipherMode))
    } catch (e) {
