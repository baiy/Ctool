...
const convert = (items: HarEntry, target: string) => {
    ...
    if (targetInfo.clientId === "custom-define-data-schema") {
        return items.log.entries.map(item => {
            const headers = Object.fromEntries(item.request.headers?.map(({name, value}) => {
                return [name, value]
            }) || [])
            const cookies = Object.fromEntries(item.request.cookies?.map(({name, value}) => {
                return [name, value];
            }) || [])
            const queryString = Object.fromEntries(item.request.queryString?.map(({name, value}) => {
                return [name, value];
            }) || [])
            const postData = (item.request.postData?.params ? Object.fromEntries(item.request.postData.params.map(({name, value}) => {
                return [name, value];
            ...
            return [
                ...
                ...(item.request.method?.toLowerCase() === "post" && postData ? [`const postData = ${JSON.stringify(postData, null, 4)};`] : []),
