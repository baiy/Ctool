<template>
    <Align direction="vertical">
        <Display position="right-center" v-for="i of range(0,7)">
            <Input v-model="outputInstance[`output${i}`].value" :placeholder="$t('time_timezone_input_placeholder')" :size="size"/>
            <template #extra>
                <Align>
                    <Select
                    ...
                    />
                    <Button v-if="isValid" :text="$t(`main_ui_copy`)" @click="$copy(outputInstance[`output${i}`].value)" :size="'small'" :type="'primary'"/>
                </Align>
            </template>
        </Display>
...
const check = (value) => {
    return (new RegExp(/^\d+-\d+-\d+ \d+:\d+:\d+$/)).test(value.trim())
