<template>
    <div class="ctool-page-option" style="margin-bottom: 5px" v-row="`1-1`">
        <Display>
            ...
            <template #extra>
                <Align>
                    <Dropdown :size="'small'" :options="getCommonExpression()" :placeholder="$t('regex_common')" @select="(value)=>action.current.input = value"/>
                    <Button :size="'small'" :type="'primary'" @click="showReference = !showReference" :text="$t(`main_ui_reference`)"/>
                </Align>
            </template>
        </Display>
        <Display>
            <Textarea :disabled="action.current.is_delete" :height="80" v-model="action.current.replace" :placeholder="$t('regex_replace_content')"/>
            <template #extra>
    ...
    <HeightResize v-slot="{height}" :append="['.ctool-page-option']" v-row="`1-1`">
        <Display>
            <Textarea :height="height" v-model="action.current.content" :placeholder="$t('regex_input')"/>
            <template #extra>
                <Align>
                    <Bool border :size="'small'" v-model="action.current.is_global" :label="$t('regex_global')"/>
                    <Bool border :size="'small'" v-model="action.current.is_ignore_case" :label="$t('regex_ignore_case')"/>
                </Align>
            </template>
        </Display>
...
watch(() => action.current, (current) => {
    ...
    try {
        ...
        const replace = !current.is_delete && current.replace === "" ? false : (current.is_delete ? "" : current.replace)

        let reg = new RegExp(
            current.input, (current.is_ignore_case ? 'i' : '') + (current.is_global ? 'g' : '')
        );
        if (replace !== false) {
