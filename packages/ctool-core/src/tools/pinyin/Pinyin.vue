        ...
        <Align direction="vertical">
            ...
            <Card class="ctool-page-option">
                <Align horizontal="center">
                    <Radio v-model="action.current.type" :options="typeLists.map(value=>{return {value,label:$t(`pinyin_${value}`)}})"/>
                    ...
                    <Bool border v-model="action.current.tone_is_number" :label="$t('pinyin_tone_is_number')" v-if="action.current.type === 'tone'"/>
                </Align>
            </Card>
    ...
    if (action.current.input.trim()) {
        try {
            return pinyin(
                ...
                action.current.delimiter === "null" ? "" : (action.current.delimiter === "blank" ? " " : action.current.delimiter),
