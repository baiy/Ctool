    ...
    const handle = () => {
        ...
        const update = (key: "locale" | "theme") => {
            ...
            if (key === "theme") {
                ...
                if (theme.value === 'auto') {
                    _theme = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? "dark" : "light"
                }
        ...
        document.getElementsByTagName('html')[0].dataset.locale = locale.value === secondLocale ? defaultLocale : secondLocale
